...
import { FalContentPolicyError, FalValidationError, FalAPIError } from "../types";
 ...
 */
export async function callFalModel<TInput extends Record<string, unknown> = Record<string, unknown>, TOutput = unknown>(
  modelId: string,
  input: TInput,
  apiKey?: string
  ...
  try {
    ...
    if (error && typeof error === 'object') {
      ...
      if (errorObj.body && errorObj.body.detail && Array.isArray(errorObj.body.detail)) {
        const detail = errorObj.body.detail[0];
        console.error('Error details:', JSON.stringify(errorObj.body.detail, null, 2));
        ...
        if (detail.type === "content_policy_violation") {
          ...
          console.error(`🚫 Content policy violation for prompt: "${rejectedPrompt}"`);
