...
export const generateTextInternal = internalAction({
  args: {
    ...
    reasoningEffort: v.optional(v.union(v.literal("minimal"), v.literal("low"), v.literal("medium"), v.literal("high"))),
    // Tool-related parameters
    tools: v.optional(v.any()), // Tools with their schemas
    toolChoice: v.optional(v.union(
      v.literal('auto'),
      v.literal('none'),
      v.literal('required')
    )),
    ...
    // Fallback providers
    fallbackProviders: v.optional(v.array(v.object({
      name: v.union(v.literal("openai"), v.literal("anthropic"), v.literal("google"), v.literal("openrouter")),
      model: v.string()
    }))),
  },
  ...
  handler: async (_ctx, args) => {
    ...
    if (args.modelPreset && result?.model) {
      console.log(`[AI SDK Actions] Model preset '${args.modelPreset}' used model: ${result.model}`);
    }
