 ...
 */
function getProvider(provider: AIProvider, reasoningEffort?: 'minimal' | 'low' | 'medium' | 'high') {
  switch (provider.name) {
    ...
    case "openrouter":
      ...
      if (provider.model.includes('gpt-5')) {
        ...
        console.log(`[AISdkClient] Creating OpenRouter client with reasoning for GPT-5, effort: ${effort}`);
...
export async function streamTextWithAI(
  ...
  const provider = params.provider || { name: "openrouter", model: "openai/gpt-5" };
  const isGPT5Model = provider.model.includes('gpt-5');
  
  try {
    const startTime = Date.now();
    let firstTokenTime: number | undefined;
    console.log(`[AI SDK Stream] Starting stream with ${provider.name}/${provider.model}`);
    ...
    const maxOutputTokens = params.maxOutputTokens ?? params.maxTokens ?? (isGPT5Model ? 2000 : 2000);
    ...
    if (isGPT5Model) {
      ...
      console.log(`[AI SDK Stream] GPT-5 configured with reasoning effort: ${params.reasoningEffort || 'low'}`);
    } else {
    ...
    for await (const chunk of result.textStream) {
      ...
      if (!firstTokenTime) {
        ...
        console.log(`[AI SDK Stream] First token received after ${firstTokenTime}ms`);
    ...
    console.log(`[AI SDK Stream] Completed in ${totalTime}ms (first token: ${firstTokenTime}ms)`);
    ...
    console.error(`[AI SDK Stream] Error with ${provider.name}/${provider.model}:`, errorMessage);
