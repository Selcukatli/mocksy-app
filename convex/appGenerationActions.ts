...
export const improveAppStoreDescription = action({
  ...
  handler: async (ctx, args): Promise<{
    ...
    try {
      ...
      await ctx.scheduler.runAfter(0, internal.appGenerationActions.processImproveAppStoreDescriptionJob, {
        jobId,
      });
...
export const processImproveAppStoreDescriptionJob = internalAction({
  ...
  handler: async (ctx, args) => {
    try {
      // Get job
      const job = await ctx.runQuery(internal.generationJobs.getJobById, {
        jobId: args.jobId,
      });
      ...
      // Fetch app
      const app = await ctx.runQuery(internal.apps.getAppByIdInternal, { appId: job.appId });
      if (!app) {
        await ctx.runMutation(internal.generationJobs.updateGenerationJobStatus, {
          jobId: args.jobId,
          status: "failed",
