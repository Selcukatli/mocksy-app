...
export const handleClerkWebhook = httpAction(async (ctx, request) => {
  ...
  switch (eventType) {
    ...
    case "user.updated": {
      ...
      try {
        await ctx.runMutation(internal.webhooks.clerk.mutations.upsertUserFromClerk, {
          userId: id,
          username: displayUsername,
          firstName: first_name ?? undefined,
          lastName: last_name ?? undefined,
    ...
    case "user.deleted": {
      const { id } = evt.data;
      await ctx.runMutation(internal.webhooks.clerk.mutations.deleteUserFromClerk, {
        userId: id,
      });
      break;
    }
